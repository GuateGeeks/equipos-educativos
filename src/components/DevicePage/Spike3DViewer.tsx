...
export default function Spike3DViewer() {
  ...
  useEffect(() => {
    let mounted = true;
    console.log('Spike3DViewer mounted');
    ...
    const load = async () => {
      try {
        console.log('Loading A-Frame and orbit-controls scripts');
        ...
        if (mounted) {
          console.log('A-Frame and orbit-controls loaded');
  ...
  useEffect(() => {
    if (isLoaded) {
      console.log('Spike3DViewer isLoaded, triggering resize');
      const timer = setTimeout(() => {
        console.log('Dispatching resize event for A-Frame scene');
